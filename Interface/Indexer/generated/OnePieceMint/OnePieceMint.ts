...
export class OnePieceMint extends ethereum.SmartContract {
  ...
  userTokenID(param0: Address): BigInt {
    let result = super.call("userTokenID", "userTokenID(address):(uint256)", [
      ethereum.Value.fromAddress(param0),
    ]);
  ...
  try_userTokenID(param0: Address): ethereum.CallResult<BigInt> {
    let result = super.tryCall(
      "userTokenID",
      "userTokenID(address):(uint256)",
      [ethereum.Value.fromAddress(param0)],
    );
    if (result.reverted) {
      return new ethereum.CallResult();

...
export class CoordinatorSet extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save CoordinatorSet entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type CoordinatorSet must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("CoordinatorSet", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): CoordinatorSet | null {
    return changetype<CoordinatorSet | null>(
      store.get_in_block("CoordinatorSet", id.toHexString()),
    );
  }

  static load(id: Bytes): CoordinatorSet | null {
    return changetype<CoordinatorSet | null>(
      store.get("CoordinatorSet", id.toHexString()),
    );
  }
  ...
  get vrfCoordinator(): Bytes {
    let value = this.get("vrfCoordinator");
    if (!value || value.kind == ValueKind.NULL) {
      throw new Error("Cannot return null for a required field.");
    } else {
      return value.toBytes();
  ...
  set vrfCoordinator(value: Bytes) {
    this.set("vrfCoordinator", Value.fromBytes(value));
  }
